...
import { Alert, Button, Card, Col, Descriptions, Row, Select, Space, Table, Tabs, Tag } from 'antd';
...
export function ImportDetailPage() {
  ...
  const summary = useQuery({
    queryKey: ['etl', importId, 'resumo'],
    queryFn: () => getImportSummary(importId),
    enabled: importId > 0,
    refetchInterval: pollingInterval,
  });
  const errors = useQuery({
    queryKey: ['etl', importId, 'erros'],
    queryFn: () => getImportErrors(importId),
    enabled: importId > 0,
    refetchInterval: pollingInterval,
  });
  const duplicates = useQuery({
    queryKey: ['etl', importId, 'duplicidades'],
    queryFn: () => getImportDuplicates(importId),
    enabled: importId > 0,
    refetchInterval: pollingInterval,
  });
  ...
  const mappingMutation = useMutation({
    ...
    onSuccess: async () => {
      AppToast.success('Mapeamento salvo e reprocessamento iniciado.');
      await refresh();
    },
  ...
  const approval = useMutation({
    ...
    onSuccess: async () => {
      AppToast.success('Carga aprovada e enfileirada.');
      await refresh();
    },
    onError: (error) => AppToast.error(normalizeApiError(error).message),
  });
  const cancellation = useMutation({
    ...
    onSuccess: async () => {
      AppToast.success('Importação cancelada.');
      await refresh();
  ...
  if (item.error)
    return <Alert type="error" showIcon message={normalizeApiError(item.error).message} />;
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <Space>
            {permissions.includes('etl.exportar') && (
              <Button
            ...
            {permissions.includes('etl.aprovar') && current?.status === 'parcial' && (
              <Button
                type="primary"
                icon={<CheckOutlined />}
            ...
            {permissions.includes('etl.editar') &&
              ['pendente', 'parcial', 'falha'].includes(current?.status ?? '') && (
                <Button danger icon={<CloseOutlined />} onClick={() => cancellation.mutate()}>
                  Cancelar
                </Button>
              )}
          </Space>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
        <Col xs={12} lg={4}>
          <Card>
            <Statistic title="Total" value={totals?.total ?? 0} />
          </Card>
        </Col>
        <Col xs={12} lg={4}>
          <Card>
            <Statistic title="Válidas" value={totals?.validas ?? 0} />
          </Card>
        </Col>
        <Col xs={12} lg={4}>
          <Card>
            <Statistic title="Inválidas" value={totals?.invalidas ?? 0} />
          </Card>
        </Col>
        <Col xs={12} lg={4}>
          <Card>
            <Statistic title="Duplicadas" value={totals?.duplicadas ?? 0} />
          </Card>
        </Col>
        <Col xs={12} lg={4}>
          <Card>
            <Statistic title="Pendentes" value={totals?.pendentes ?? 0} />
          </Card>
        </Col>
        <Col xs={12} lg={4}>
          <Card>
            <Statistic title="Carregadas" value={totals?.carregadas ?? 0} />
          </Card>
        </Col>
      </Row>
      ...
      <Tabs
        items={[
                    ...
                    { title: 'Coluna do arquivo', render: (_, value) => value },
                    {
                      ...
                      render: (_, value) => (
                        <Select
                          allowClear
                          style={{ width: 260 }}
                          defaultValue={current?.mapeamento_colunas[value]}
                          options={targetFields}
                          onChange={(selected) =>
                            setMapping((old) => ({ ...old, [value]: selected }))
                          }
                        />
                      ),
                    },
          ...
          {
            key: 'errors',
            label: `Erros e avisos (${formatInteger(errors.data?.length)})`,
            children: (
              <Table
                rowKey="id"
                dataSource={errors.data ?? []}
                columns={[
                  { title: 'Linha', dataIndex: 'numero_linha', render: formatInteger },
                  { title: 'Severidade', dataIndex: 'severidade' },
                  { title: 'Campo', dataIndex: 'campo' },
                  { title: 'Valor', dataIndex: 'valor' },
                  { title: 'Motivo', dataIndex: 'mensagem' },
                ]}
              />
            ),
          },
          {
            key: 'duplicates',
            label: `Duplicidades (${formatInteger(duplicates.data?.length)})`,
            children: (
              <Table
                rowKey="id"
                dataSource={duplicates.data ?? []}
                columns={[
                  { title: 'Staging', dataIndex: 'staging_pessoa_id' },
                  { title: 'Pessoa candidata', dataIndex: 'pessoa_candidata_id' },
                  { title: 'Regra', dataIndex: 'criterio' },
                  {
                    title: 'Score',
                    dataIndex: 'score',
                    render: (value: number | string) => formatNumber(value),
                  },
                  { title: 'Decisão', dataIndex: 'decisao' },
                ]}
              />
            ),
          },
