...
import {
  Button, Card, DatePicker, Descriptions, Form, Input, Modal, Select, Space,
...
import {
  addDemandAttendance, changeDemandStatus, getDemand, listDemandCatalog,
  listDemandResponsibles, updateDemand,
...
export function DemandDetailPage() {
  ...
  const demand = useQuery({
    queryKey: ['demandas', id], queryFn: () => getDemand(id), enabled: id > 0,
  });
      ...
      if (modal === 'status') {
        return changeDemandStatus(id, {
          ...
          resultado_atendimento_id: values.resultado_atendimento_id
            ? Number(values.resultado_atendimento_id) : undefined,
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={permissions.includes('demandas.editar') && <Space>
          <Button icon={<EditOutlined />} onClick={() => setModal('status')}>Alterar status</Button>
          <Button icon={<UserSwitchOutlined />} onClick={() => setModal('assign')}>Atribuir</Button>
          <Button type="primary" icon={<PlusOutlined />}
            onClick={() => setModal('attendance')}>Registrar atendimento</Button>
        </Space>}
      />
      {item?.vencida && <Card style={{ marginBottom: 16, borderColor: '#ff4d4f' }}>
      ...
      <Tabs items={[
        {
          key: 'data', label: 'Dados', children: <Card loading={demand.isLoading}>
            <Descriptions bordered column={{ xs: 1, md: 2 }}>
              <Descriptions.Item label="Status"><Tag>{item?.status_nome}</Tag></Descriptions.Item>
              <Descriptions.Item label="Categoria">{item?.categoria_nome || '—'}</Descriptions.Item>
              <Descriptions.Item label="Solicitante">{item?.solicitante_nome || '—'}</Descriptions.Item>
              <Descriptions.Item label="Território">{item?.territorio_nome || '—'}</Descriptions.Item>
              <Descriptions.Item label="Responsável">{item?.responsavel_nome || 'Não atribuído'}</Descriptions.Item>
              <Descriptions.Item label="Prazo">{item?.prazo || 'Sem prazo'}</Descriptions.Item>
              <Descriptions.Item label="Resultado">{item?.resultado_nome || '—'}</Descriptions.Item>
              <Descriptions.Item label="Descrição" span={2}>{item?.descricao}</Descriptions.Item>
            </Descriptions>
          </Card>,
        },
        {
          key: 'attendance', label: `Atendimentos (${item?.atendimentos.length ?? 0})`,
          children: <Table rowKey={(row) => String(row.id)} dataSource={item?.atendimentos ?? []}
            columns={[
        ...
        {
          key: 'movements', label: `Movimentações (${item?.movimentacoes.length ?? 0})`,
          children: <Table rowKey={(row) => String(row.id)} dataSource={item?.movimentacoes ?? []}
            columns={[
      ...
      <Modal open={modal !== null}
        title={modal === 'status' ? 'Alterar status' : modal === 'assign'
          ? 'Atribuir responsável e prazo' : 'Registrar atendimento'}
        okText="Salvar" confirmLoading={action.isPending} onCancel={() => setModal(null)}
        ...
        <Form form={form} layout="vertical" onFinish={(values) => action.mutate(values)}>
          ...
          {modal === 'assign' && <>
            <Form.Item name="responsavel_atendimento_id" label="Responsável"
              rules={[{ required: true }]}><Select options={options(responsibles.data)} /></Form.Item>
            <Form.Item name="prazo" label="Prazo"
              initialValue={item?.prazo ? dayjs(item.prazo) : undefined}>
              <DatePicker style={{ width: '100%' }} />
            </Form.Item>
            <Form.Item name="observacao" label="Observação"><Input.TextArea /></Form.Item>
          </>}
          {modal === 'attendance' && <>
            <Form.Item name="responsavel_atendimento_id" label="Responsável">
            ...
            <Form.Item name="descricao" label="Ação realizada" rules={[{ required: true, min: 2 }]}>
              <Input.TextArea />
            </Form.Item>
          </>}
        </Form>
      </Modal>
      <Button type="link" onClick={() => navigate('/demandas')}>Voltar para demandas</Button>
    </div>
  );
}
