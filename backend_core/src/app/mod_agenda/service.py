...
class AgendaService:
    ...
    async def update_catalog(
        ...
        item = await self.repository.update_catalog(
            table, actor.tenant_id, item_id, payload
        )
    ...
    async def list_events(
        ...
        **filters: Any,
        ...
        return await self.repository.list_events(
            actor.tenant_id, accessible_ids=ids, **filters
        )
    ...
    async def cancel_event(
        ...
        await self.repository.cancel_event(
            actor.tenant_id, event_id, actor.user_id, reason.strip()
        )
        item = await self.repository.get_event(actor.tenant_id, event_id)
        assert item is not None
        await self._audit(actor, "editar", "evento", event_id, current, item)
    ...
    async def detail(
        ...
        return EventDetailResponse.model_validate(
            {
                **item,
                "participantes": await self.repository.participants(
                    actor.tenant_id, event_id
                ),
                "liderancas": await self.repository.leaderships(
                    actor.tenant_id, event_id
                ),
                "convites": await self.repository.invitations(
                    actor.tenant_id, event_id
                ),
                "pautas": await self.repository.agenda_items(
                    actor.tenant_id, event_id
                ),
                "presenca": await self.repository.attendance(
                    actor.tenant_id, event_id
                ),
                "demandas": await self.repository.demands(
                    actor.tenant_id, event_id
                ),
                "lembretes": await self.repository.reminders(
                    actor.tenant_id, event_id
                ),
                "insights": await self.repository.insights(
                    actor.tenant_id, event_id
                ),
            }
        )
    ...
    async def add_participant(
        ...
        item = await self.repository.upsert_participant(
            actor.tenant_id, event_id, payload
        )
    ...
    async def add_leadership(
        ...
        item = await self.repository.upsert_leadership(
            actor.tenant_id, event_id, payload
        )
        await self._audit(
            actor,
    ...
    async def remove_leadership(
        ...
        await self.ensure_event(actor, access, event_id, administer=True)
        await self.repository.delete_leadership(
            actor.tenant_id, event_id, leadership_id
        )
        await self._audit(
            actor,
            "excluir",
    ...
    async def create_invitation(
        ...
        item = await self.repository.create_invitation(
            actor.tenant_id, event_id, payload
        )
    ...
    async def create_agenda_item(
        ...
        await self.ensure_event(actor, access, event_id, administer=True)
        item = await self.repository.create_agenda_item(
            actor.tenant_id, event_id, payload
        )
    ...
    async def create_demand(
        ...
        if payload.categoria_demanda_id:
            await self._require_reference(
                actor, "categoria_demanda", payload.categoria_demanda_id
            )
        ...
        if payload.territorio_id:
            await self._ensure_territory(
                actor, access, payload.territorio_id, administer=True
            )
    ...
    async def _validate_event_references(
        ...
        *,
        ...
        if territory_id:
            await self._ensure_territory(
                actor, access, territory_id, administer=administer
            )
        ...
        for table, item_id in (
            ...
            if item_id and await self.repository.get_catalog(
                table, actor.tenant_id, item_id
            ) is None:
    ...
    async def _ensure_territory(
        ...
        *,
        ...
        territory = await self.territories.get_territory(
            actor.tenant_id, territory_id
        )
        ...
        if administer and not access.unrestricted:
            ...
            admin_ids = await self.territories.accessible_ids(
                actor.tenant_id, admin_access
            )
    ...
    async def _require_reference(
        self, actor: RequestActor, table: str, item_id: int
    ) -> None:
