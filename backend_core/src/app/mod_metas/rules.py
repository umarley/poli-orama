"""Regras canonicas compartilhadas pelo dominio de metas."""

from decimal import ROUND_HALF_UP, Decimal
from typing import Any

ACTIVE_GOAL_STATUSES = frozenset({"ativa", "em_risco"})
...
def ranking_score(
    attainment: Decimal, registrations: int, average_engagement: Decimal
) -> Decimal:
    return (
        min(attainment, Decimal("100")) * RANKING_ATTAINMENT_WEIGHT
        + min(Decimal(registrations), Decimal("100"))
        * RANKING_REGISTRATIONS_WEIGHT
        + min(average_engagement * Decimal("10"), Decimal("100"))
